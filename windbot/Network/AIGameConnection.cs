...
namespace WindBot.Game.Network
{
	public class AIGameConnection
	{
		...
		public AIGameConnection(IPAddress address, int port)
		{
		...
		{
			return (_client.Client.Poll(20, SelectMode.SelectRead) && _client.Available == 0);
		}
			...
			try
			{
				...
				while (IsConnected)
				{
					if(CheckDisconnected()){
						Logger.WriteLine("Server is close");
						Close();
					}
					InternalTick();
